...
function gridReload(gridName,httpParams,selectTypeName){
	...
	$("#"+gridName).datagrid("options").url = contextPath+'/goods/operateNewGoodsApply/queryNewGoodsApply';
        ...
        columns:[[
             ...
             {field:'skuName',title:'商品名称',width:'200px',align:'left',
             	formatter: function(value,row,index){
                     return "<a href='#' onclick=\"openDialog('"+contextPath+"/goods/operateNewGoodsApply/updateGoodsView?id="+row.id+"','新品申请','edit','"+row.id+"')\" class='ualine'>"+value+"</a>";
             	}
              },
...
function goodsSearch(){
    ...
    $("#gridArchives").datagrid('options').url=contextPath+'/goods/operateNewGoodsApply/queryNewGoodsApply';
...
function addGoodsView(){
	...
	if(categoryCode){
		if(categoryCode.length==6){
			...
			openDialog(contextPath+"/goods/operateNewGoodsApply/addGoodsView","新品申请","add",obj);
		}else{
			var obj = $.extend({},param);
				obj.categoryName="";
			openDialog(contextPath+"/goods/operateNewGoodsApply/addGoodsView","新品申请","add",obj);
...
function copyGoodsView(){
    ...
    openDialog(contextPath+"/goods/operateNewGoodsApply/addGoodsView","新品申请","copy",""+escape(selectionRow)+"");
	setTimeout(function(){
//    		window.location.href = contextPath+"/goods/operateNewGoodsApply/addGoodsView?data="+escape(selectionRow);
	},10);
...
function toImportproduct(){
    var param = {
        url:contextPath+"/goods/operateNewGoodsApply/importList",
        tempUrl:contextPath+"/goods/operateNewGoodsApply/exportTemp",
        type:1,
    }
...
//模板导出
function exportTemp(){
	location.href=contextPath+'/goods/operateNewGoodsApply/exportTemp';
}
