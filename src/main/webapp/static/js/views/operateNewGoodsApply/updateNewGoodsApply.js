...
function getGoodsArchivesDetail(id){
	var args = {}
	var httpUrl = contextPath+"/goods/operateNewGoodsApply/getGoodsSkuById?id="+id;
...
//提交表单
function submitForm(){
	var url = contextPath+'/goods/operateNewGoodsApply/updateGoods';
...
function goodsAddView(){
	closeDialog();
	openDialog(contextPath+"/goods/operateNewGoodsApply/addGoodsView","新增商品档案","add","");
}
...
function copyAddGoodsView(){
	...
	closeDialog();
	openDialog(contextPath+"/goods/operateNewGoodsApply/addGoodsView","新增商品档案","copy",""+escape(selectionRow)+"");

//	setTimeout(function(){
//	var data = ''+JSON.stringify(updateSku)+'';
//	var newUrl = contextPath+"/goods/operateNewGoodsApply/addGoodsView?data="+escape(data);
