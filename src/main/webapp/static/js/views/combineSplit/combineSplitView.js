...
function initDatacombineSplitView(){
    gridHandel.setGridName("combineSplitView");
    gridHandel.initKey({
        firstName:'skuCode',
        enterName:'skuCode',
...
function setDataValue(data) {
    ...
    $("#combineSplitView").datagrid("loadData",newRows);
    setTimeout(function(){
...
// 库存调整为负数
function selectTion(){
	var rows = $("#combineSplitView").datagrid('getRows');
	var selectVal=$("#io").combobox('getValue');
	...
	$("#combineSplitView").datagrid("loadData", rows);
}
//保存
function saveOrder(){
	// 机构id
...
function updateListData(data){
	   /* var nowRows = gridHandel.getRowsWhere({skuCode:'1'});
	    ...
	    $("#combineSplitView").datagrid("loadData",data);
	}
