...
function gridReload(gridName,httpParams,selectTypeName){
	...
	$("#"+gridName).datagrid("options").url = contextPath+'/common/operateGoods/queryGoodsSku';
        ...
        columns:[[
             ...
             {field:'skuCode',title:'货号',width:'120px',align:'left',
            	formatter: function(value,row,index){
                    return "<a href='#' onclick=\"openDialog('"+contextPath+"/common/operateGoods/updateGoodsView?id="+row.id+"','修改商品档案','edit','"+row.id+"')\" class='ualine'>"+value+"</a>";
            		//return "<a href="+contextPath+"/common/operateGoods/updateGoodsView?id="+row.id+" class='ualine'>"+value+"</a>";
            	}
             },
...
function goodsSearch(){
    ...
    var formParams = $("#formGoodsArchives").serializeObject();
    $("#gridArchives").datagrid('options').url=contextPath+'/common/operateGoods/queryGoodsSku';
...
function addGoodsView(){
	...
	if(categoryCode){
		if(categoryCode.length==6){
			...
			openDialog(contextPath+"/common/operateGoods/addGoodsView","新增商品档案","add",obj);
		}else{
			var obj = $.extend({},param);
				obj.categoryName="";
			openDialog(contextPath+"/common/operateGoods/addGoodsView","新增商品档案","add",obj);
...
var  dalogTemp;
var dialogHeight = 630;//$(window).height()*(4/5);
var dialogWidth = 930;//$(window).width()*(5/9);
...
function copyGoodsView(){
	if($("#gridArchives").datagrid("getSelections").length <= 0){
        ...
        openDialog(contextPath+"/common/operateGoods/addGoodsView","新增商品档案","copy",""+escape(selectionRow)+"");
    	setTimeout(function(){

    		//window.location.href = contextPath+"/common/operateGoods/addGoodsView?data="+escape(selectionRow);
    	},10);
    }
}
